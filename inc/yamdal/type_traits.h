 ...
 */
   ...
   using ndim_constant =
      std::integral_constant<ndim_t,ndim>;

   template<grid_t grid>
   using grid_constant =
      std::integral_constant<grid_t,grid>;
 ...
 */
   ...
   template<ndim_t ndim,
            grid_t grid>
   struct is_index_type<index<ndim,grid>>
      : std::true_type {};

// helper variable template
   template<typename A>
   inline constexpr bool is_index_type_v =
      is_index_type<A>::value;
 ...
 */
   // false
   template<typename A>
   struct has_ndim_member
      : std::false_type {};

   // true
   template<typename A>
      requires requires(){ { (std::remove_reference_t<A>::ndim) } -> std::same_as<const ndim_t&>; }
   struct has_ndim_member<A>
      : std::true_type {};

// helper variable template
   template<typename A>
   inline constexpr bool has_ndim_member_v =
      has_ndim_member<A>::value;
 ...
 */
   // false
   template<typename A>
   struct has_grid_member
      : std::false_type {};

   // true
   template<typename A>
      requires requires(){ { (std::remove_reference_t<A>::grid) } -> std::same_as<const grid_t&>; }
   struct has_grid_member<A>
      : std::true_type {};

// helper variable template
   template<typename A>
   inline constexpr bool has_grid_member_v =
     has_grid_member<A>::value;
 ...
 */
   ...
   struct has_index_type_member<A>
      : std::true_type {};

// helper variable template
   template<typename A>
   inline constexpr bool has_index_type_member_v =
      has_index_type_member<A>::value;
 ...
 */
   // false
   template<typename A>
   struct has_element_type_member
      : std::false_type {};

   // true
   template<typename A>
      requires requires(){ typename std::remove_reference_t<A>::element_type; }
   struct has_element_type_member<A>
      : std::true_type {};

// helper variable template
   template<typename A>
   inline constexpr bool has_element_type_member_v =
      has_element_type_member<A>::value;
...
// specialisation with helper member
   ...
   struct ndim_of<A>
      : ndim_constant<std::remove_reference_t<A>::ndim> {};
...
// helper variable template
   template<typename A>
   inline constexpr ndim_t ndim_of_v =
      ndim_of<A>::value;
...
// helper variable template
   template<typename A>
   inline constexpr bool has_ndim_v =
      has_ndim<A>::value;
...
// specialisation with helper member
   ...
   struct grid_of<A>
      : grid_constant<std::remove_reference_t<A>::grid> {};
...
// helper variable template
   template<typename A>
   inline constexpr grid_t grid_of_v =
      grid_of<A>::value;
...
// helper variable template
   template<typename A>
   inline constexpr bool has_grid_v =
      has_grid<A>::value;
...
// specialisation with helper member
   ...
   struct index_type_of<A>
      : std::type_identity<typename std::remove_reference_t<A>::index_type> {};
...
// helper type template
   template<typename    A0,
            typename... As>
      requires (has_index_type_v<A0>)
            && (has_index_type_v<As>&&...)
            && (std::same_as<index_type_of_t<A0>,
                             index_type_of_t<As>>&&...)
 ...
 */
   template<typename    A,
      ...
      requires has_ndim_v<A>
            && ((has_ndim_v<As>)&&...)
   inline constexpr bool has_same_ndim_v =
      ((ndim_of_v<A> == ndim_of_v<As>)&&...);
 ...
 */
   template<typename    A,
      ...
      requires has_grid_v<A>
            && ((has_grid_v<As>)&&...)
   inline constexpr bool has_same_grid_v =
      ((grid_of_v<A> == grid_of_v<As>)&&...);
 ...
 */
   ...
   struct element_type_with
      : std::type_identity<std::invoke_result_t<A,index<ndim,grid>>> {};

// helper type template
   template<typename  A,
            ndim_t ndim,
            grid_t grid= primal>
   using element_type_with_t =
      typename element_type_with<A,ndim,grid>::type;
 ...
 */
   ...
   struct element_type_of
      : std::type_identity<std::invoke_result_t<A,index_type_of_t<A>>> {};

   template<typename A>
   using element_type_of_t =
      typename element_type_of<A>::type;
}
